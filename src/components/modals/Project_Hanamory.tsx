  ...
  return (
    <div className='animate-fadeIn'>
      ...
      <div className='text-[16px] mb-2'>
        <div className='m-2'>· 대학 졸업작품으로 진행하는 1인 풀스택 프로젝트에요.</div>
        <div className='m-2'>
          · Hanamory는 꽃다발 이미지를 업로드하면, AI가 꽃의 종류를 인식해 정보와 꽃말을 SNS 공유가 가능한 카드 형태로 제공하는 서비스에요.YOLOv5 커스텀 학습과, Docker 기반 EC2 클라우드 배포까지 개발
          중이에요.
        </div>
      </div>
      <SectionLine />
      ...
      <div className='text-[16px] mb-2'>
        <div className='m-2'>
          · 커스텀 트레이닝을 통한 YOLOv5 모델의 꽃다발 추론 API를 제공해요. API를 통해 꽃다발 이미지에서 꽃의 종류를 인식하고, Gemini 모델을 활용해 해당 꽃의 정보와 꽃말을 제공해요.
        </div>
        <div className='m-2'>· 카드형태의 SNS 공유 기능, 위치기반 주변 꽃집 정보 등, 플로워에게 친화적인 다양한 기능을 추가할 예정이에요.</div>
      </div>
      <SectionLine />
      ...
      <div className='ml-2'>
        1인 개발이기에 나태해지지 않도록 일주일 단위로 개발 목표를 정해두고, 계획 한 일정에 맞추어 최대한 개발일정을 맞췄어요. AI모델 학습도 처음, 학습한 모델을 Docker로 배포하는 것도 처음이었기에
        빠르게 공부하며 개발했어요. 개발 진행상황은 모두
        <a
      ...
      <div className='text-[16px] font-bold mt-5 m-2'>MLOps</div>

      <div className='m-2'>
        · Roboflow를 활용해 꽃 이미지 데이터를 수집하고 직접 라벨링한 뒤, 다양한 하이퍼파라미터로 여러 YOLOv5 모델을 반복 학습·평가했어요. F1 Score, mAP 등 성능 지표를 기반으로 가장 추론 성능이 우수한
        모델을 선정했어요.
      </div>
      <div className='m-2'>· Flask API 서버와 학습된 YOLOv5 모델을 Docker로 하나의 이미지로 패키징하여, 환경 의존성 문제를 해결하고 배포를 자동화했어요.</div>
      <div className='m-2'>· 완성된 Docker 이미지를 AWS EC2 서버에 배포하고, 실 서비스 환경에서 안정적으로 운영할 수 있도록 전체 파이프라인을 직접 설계·구현했어요.</div>

      <div className='text-[16px] font-bold mt-5 ml-2'>트러블 슈팅</div>
      <div className='m-2'> · Windows에서 학습한 모델의 경로 정보(WindowsPath)를 리눅스 빌드 환경에서는 인식하지 못해, 모델 파일을 직접 재저장/수정하여 문제를 해결했어요.</div>
      <div className='m-2'>
        · Vercel 환경에서 프론트엔드는 https로 서비스되고 있지만, 백엔드 Flask API 서버는 http로 요청이 이루어져 mixed-content로 인한 브라우저 보안 에러가 발생했어요. 단순히 보안 설정을 끄는 임시
        해결책은 안전하지 않다고 판단해, Nginx를 이용한 리버스 프록시를 적용해 Flask API 서버에도 https 인증서를 적용했어요.
      </div>
    </div>
  )
}
